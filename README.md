...
- RSS订阅地址写入 `00_Inbox/清简首页-RSS订阅.md`，可随普通笔记在 Windows 与 iOS 间同步；文章仅在收藏后写入 `12_RSS收藏`
- 自动扫描库内现有 Markdown 待办并汇总；新待办记录创建日期
- 仅“急 / 缓”两级待办，优先级直接用加粗彩色文字呈现；旧待办默认归为“缓”
- 完成待办后移入 `10_已完成待办/已完成待办.md`，记录创建和完成日期，并可在首页查看
- 每日瞬间支持标题/关键词和手机图片，图片自动压缩；归档到文件夹时按“日期-标题”生成笔记，归档到现有笔记时以“时间＋标题”追加
- 优质内容收集：按正文顺序提取文字与段落图片，也可直接粘贴，并保存为独立笔记
- 各输入模块独立保留未保存草稿，保存或刷新其他模块时不会丢失
